...
const getNextSignal = async ({ stateOfRead, metaFile }) => {
  ...
  if (signal.length < SIGNAL_TYPE_LENGTH || !signalDetails) {
    ...
    return await getNextSignal({ stateOfRead, metaFile })
  }

  return signal
}

const readDataType = async ({ stateOfRead, metaFile, buffer }) => {
...
const readStructure = async ({ stateOfRead, metaFile }) => {
  ...
  structureLength = buffer.slice(0,1).readUIntBE(0, 1)
  structureRepeat = buffer.slice(1,3).readUIntBE(0, 2)
  return { structureLength, structureRepeat }
}

const readValues = async ({ stateOfRead, metaFile, dataTypeDetails, structure, dataType }) => {
...
async function* readSignals(metaFile, stateOfRead) {
  ...
  while (signal !== false) {
    ...
    if (!isSpecial(dataType)) value = await readValues({ stateOfRead, metaFile, dataTypeDetails, structure, dataType })
    if (Array.isArray(value) && value.length === 1) value = value[0]
    // TODO: remove counter to read the entire file
    if (signal === 'DEVC') devcCount ++
    if (devcCount == 2) signal = false
    yield { signal, stateOfRead: { ...stateOfRead }, dataType, structure, value }
  }
}
...
const pileUp = (parts) => {
  ...
  parts.reverse().forEach((part, index) => {
    ...
    if (lastPart) {
      batchSize += lastPart.stateOfRead.position - part.stateOfRead.position
